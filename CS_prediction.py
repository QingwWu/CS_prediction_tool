...
if st.button("预测"):
    ...
    if predicted_class == 1:
        ...
        if predicted_proba[predicted_class] > 0.8:
            advice = (f"根据我们的模型，您患隐源性卒中的风险很低。" f"该模型预测您不患隐源性卒中的概率是{probability:.1f}%。" "然而，保持健康的生活方式仍然非常重要。" 
                   "我建议定期检查以监测你的心脏健康，"  "如果您出现任何症状，请及时就医。")
        else:
            advice = (f"根据我们的模型，您患隐源性卒中的风险较低。" f"该模型预测您不患隐源性卒中的概率是{probability:.1f}%。" "然而，保持健康的生活方式仍然非常重要。" 
                   "我建议定期检查以监测你的心脏健康，"  "如果您出现任何症状，请及时就医。")
    st.write(advice)

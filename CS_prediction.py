...
if st.button("预测"):
    ...
    if predicted_class == 1:
        if predicted_proba[predicted_class] >0.8:
            advice = (f"根据我们的模型，你患隐源性卒中的风险很高。" f"该模型预测你患隐源性卒中的概率为 {probability:.1f}%。"  
                  "虽然这只是一个估计，但它表明你可能面临重大风险。" "我建议你尽快咨询神经内科专家，以进行进一步评估" "确保您得到准确的诊断和必要的治疗。")
        elif predicted_proba[predicted_class] >0.7:
            advice = (
                f"根据我们的模型，你患隐源性卒中的风险较高。" f"该模型预测你患隐源性卒中的概率为 {probability:.1f}%。"
                "虽然这只是一个估计，但它表明你可能面临较大风险。" "我建议你尽快咨询神经内科专家，以进行进一步评估" "确保您得到准确的诊断和必要的治疗。")
        else :
            advice = (
                f"根据我们的模型，你患隐源性卒中的风险略高。" f"该模型预测你患隐源性卒中的概率为 {probability:.1f}%。 "
                "虽然这只是一个估计，但它表明你可能面临风险。" "我建议你尽快咨询神经内科专家，以进行进一步评估" "确保您得到准确的诊断和必要的治疗。")
    else:
        if predicted_proba[predicted_class] > 0.8:
            advice = (f"根据我们的模型，你患隐源性卒中的风险很低。" f"该模型预测你不患隐源性卒中的概率是{probability:.1f}%。" "然而，保持健康的生活方式仍然非常重要。" 
                   "我建议定期检查以监测你的心脏健康，"  "如果您出现任何症状，请及时就医。")
        else:
            advice = (f"根据我们的模型，你患隐源性卒中的风险较低。" f"该模型预测你不患隐源性卒中的概率是{probability:.1f}%。" "然而，保持健康的生活方式仍然非常重要。" 
                   "我建议定期检查以监测你的心脏健康，"  "如果您出现任何症状，请及时就医。")
    st.write(advice)
